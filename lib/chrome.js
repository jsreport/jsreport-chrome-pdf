...
function execute (reporter, definition, strategyCall, imageExecution) {
  ...
  return async (req, res) => {
    ...
    if (imageExecution && req.template.chromeImage.url) {
      htmlUrl = req.template.chromeImage.url
    } else if (!imageExecution && req.template.chrome.url) {
      htmlUrl = req.template.chrome.url
    } else {
