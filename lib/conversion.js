...
module.exports = async ({ reporter, getBrowser, getPage, htmlUrl, strategy, timeout, req, imageExecution, options }) => {
  ...
  const conversionResult = await runWithTimeout(async (executionInfo, reject) => {
    ...
    const page = getPage ? await getPage(browser) : await browser.newPage()
