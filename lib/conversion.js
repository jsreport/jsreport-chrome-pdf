...
module.exports = async ({ reporter, getBrowser, htmlUrl, strategy, timeout, req, imageExecution, options }) => {
  ...
  const conversionResult = await runWithTimeout(async (executionInfo, reject) => {
    ...
    page.on('pageerror', (err) => {
      reporter.logger.warn(`Page error: ${err.message}${err.stack ? ` , stack: ${err.stack}` : ''}`, { timestamp: new Date().getTime(), ...req })
    })
    ...
    page.on('request', (r) => {
      ...
      if (r.redirectChain().length > 0) {
        detail = ` (redirect from: ${r.redirectChain().slice(-1)[0].url()})`
      }

      reporter.logger.debug(`Page request: ${r.method()} (${r.resourceType()}) ${r.url()}${detail}`, { timestamp: new Date().getTime(), ...req })
    })
    ...
    page.on('requestfinished', (r) => {
      ...
      const log = `Page request ${requestDesc}: ${r.method()} (${r.resourceType()})${status} ${r.url()}${detail}`
      const logContext = { timestamp: new Date().getTime(), ...req }

      if (requestDesc === 'failed') {
        reporter.logger.warn(log, logContext)
      } else {
        reporter.logger.debug(log, logContext)
      }
    })

    page.on('requestfailed', (r) => {
      reporter.logger.warn(`Page request failed: ${r.method()} (${r.resourceType()}) ${r.url()}, failure: ${r.failure().errorText}`, { timestamp: new Date().getTime(), ...req })
    })
